...
#print(enumerate(trainloader)[0][1])
  ...
  for i, (input, target, template) in enumerate(trainloader):
    ...
    for choice in range(choose_tr):
      for j, tg in enumerate(target):
        ...
        template_new = template_new.cuda(non_blocking=True)
    ...
    choose_sup = 1
    count_new = 0      
    recon_temp_sup = torch.zeros(choose_sup*input.shape[0],3,input.shape[2],input.shape[3]).cuda(non_blocking=True)
    temp_trans_sup = torch.zeros(choose_sup*input.shape[0],3,input.shape[2],input.shape[3]).cuda(non_blocking=True)
    template_sup = torch.zeros(choose_sup*input.shape[0],3,input.shape[2],input.shape[3]).cuda(non_blocking=True)
    feat_sem_sup = torch.zeros(choose_sup*input.shape[0],int(feature_channel/2),input.shape[2],input.shape[3]).cuda(non_blocking=True)
    temp_sem_sup = torch.zeros(choose_sup*input.shape[0],int(feature_channel/2),input.shape[2],input.shape[3]).cuda(non_blocking=True)
    target_sup = torch.zeros(choose_sup*input.shape[0]).long()
    feature_sup = torch.zeros(choose_sup*input.shape[0],int(feature_channel),input.shape[2],input.shape[3]).cuda(non_blocking=True)
    ...
    for choice in range(choose_sup):
      for j, tg in enumerate(target):
        ...
        temp_trans_new = temp_trans_new.cuda(non_blocking=True)
        template_new = template_new.cuda(non_blocking=True)
        tg_new = torch.Tensor([cls]).long()
    ...
    target_exc = target_exc.cuda(non_blocking=True)
    target_sup = target_sup.cuda(non_blocking=True)
